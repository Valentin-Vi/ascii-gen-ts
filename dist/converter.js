...
function convertFrame(pixels, frameWidth, frameHeight, options) {
    ...
    if (frameWidth <= 0 || frameHeight <= 0) {
        throw new RangeError(`frameWidth and frameHeight must be positive, got ${frameWidth}x${frameHeight}`);
    }
    if (pixels.length < frameWidth * frameHeight * 4) {
        throw new RangeError(`pixels buffer too small: expected at least ${frameWidth * frameHeight * 4} bytes`);
    }
